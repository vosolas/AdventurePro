...
async function main() {
  ...
  try {
    ...
    try {
      const spinPurchasedEvents = await luckySpinFHE.queryFilter(
        luckySpinFHE.filters.SpinPurchased(),
        fromBlock
      );
      console.log(`🛒 Spin Purchased Events: ${spinPurchasedEvents.length}`);

      const spinCompletedEvents = await luckySpinFHE.queryFilter(
        luckySpinFHE.filters.SpinCompleted(),
        fromBlock
      );
      console.log(`🎰 Spin Completed Events: ${spinCompletedEvents.length}`);

      const gmTokensBoughtEvents = await luckySpinFHE.queryFilter(
        luckySpinFHE.filters.GmTokensBought(),
        fromBlock
      );
      console.log(`🪙 GM Tokens Bought Events: ${gmTokensBoughtEvents.length}`);

      const dailyGmCompletedEvents = await luckySpinFHE.queryFilter(
        luckySpinFHE.filters.DailyGmCompleted(),
        fromBlock
      );
